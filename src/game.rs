...
impl<'a> GameState<'a> {
    ...
    fn render_3d_view(&mut self) {
        ...
        let ceiling_color = Color::new(100, 95, 60, 255);
        ...
        for (ray_index, hit) in ray_hits.iter().enumerate() {
            ...
            // Sample texture
            let tex_x = (hit.wall_x * self.textures.texture_size as f32) as usize;

            // Pre-calculate floor color with fog (once per ray instead of per pixel)
            let floor_base = floor_texture[0];
            let floor_color = self.effects.apply_fog(floor_base, hit.distance);
            ...
            for x in x_start..x_end {
                // Draw ceiling - use optimized vertical line instead of loop
                if draw_start > 0 {
                    self.framebuffer.draw_vertical_line(x, 0, draw_start, ceiling_color);
                }
                ...
                self.framebuffer.draw_textured_line(
                    ...
                    draw_end,
                    wall_texture,
                    self.textures.texture_size,
                    self.textures.texture_size,
                    tex_x,
                    total_shade,
                );

                // Draw floor - use optimized vertical line with pre-calculated color
                if draw_end < self.framebuffer.height {
                    self.framebuffer.draw_vertical_line(x, draw_end, self.framebuffer.height, floor_color);
                }
            }
        }
    }
